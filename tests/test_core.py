...
class GanttTests(unittest.TestCase):
    def test_sidebar_reminder_control_has_bounded_styles(self) -> None:
        css = (app.STATIC_DIR / "css" / "app.css").read_text(encoding="utf-8")
        alert_rule = css.split(".nav-focus-alert {", 1)[1].split("}", 1)[0]
        icon_rule = css.split(".nav-focus-alert svg {", 1)[1].split("}", 1)[0]

        self.assertIn("width: 28px", alert_rule)
        self.assertIn("height: 28px", alert_rule)
        self.assertIn("width: 18px", icon_rule)
        self.assertIn("height: 18px", icon_rule)
        self.assertIn(".nav-focus-alert .nav-count", css)
