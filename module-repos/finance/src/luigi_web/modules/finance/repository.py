...
def backup_payload() -> dict[str, Any]:
    ...
    tables = (
        ...
        "finance_audit_events", *plans.BACKUP_TABLES,
    )
    with connect() as connection:
...
def restore_backup(payload: dict[str, Any]) -> dict[str, int]:
    ...
    plan_rows, stream_rows = plans.validate_backup(
        tables.get("finance_plans", []), tables.get("finance_income_streams", []),
    )
    plans.init_db()
    counts: dict[str, int] = {}
    with connect(write=True) as connection:
        plans.validate_restore_capacity(connection, plan_rows, stream_rows)
        ...
        for table, rows in zip(plans.BACKUP_TABLES, (plan_rows, stream_rows)):
            columns = plans.BACKUP_COLUMNS[table]
            placeholders = ",".join("?" for _ in columns)
            updates = ",".join(f"{column}=excluded.{column}" for column in columns if column != "id")
            for row in rows:
                connection.execute(
                    f"INSERT INTO {table} ({','.join(columns)}) VALUES ({placeholders}) ON CONFLICT(id) DO UPDATE SET {updates}",
