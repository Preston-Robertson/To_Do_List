...
from . import importer as cards_importer
from . import pokemon as cards_pokemon
...
def _asset_version() -> str:
    ...
    directory = Path(__file__).resolve().parent / "static" / "legacy"
    paths = [directory / "css" / "cards.css", directory / "js" / "cards.js"]
...
def _deck_cards_partial(request: Request, game_code: str, deck_id: int) -> Response:
    ...
    return _render(
        ...
        _ctx(
            request, game_code, page_title=state["deck"]["name"],
            refresh_summary=True, **state,
        ),
    )
