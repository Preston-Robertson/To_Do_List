...
class SafeWorkspace:
    ...
    def _path(self, value: Any, *, write: bool = False) -> Path:
        ...
        if any(
            "credential" in part.lower() or part.lower().endswith(".env")
        ...
        if write:
            ...
            ) or any(resolved.startswith(prefix) for prefix in PROTECTED_PREFIXES)
            if protected:
...
Read AGENTS.md and the nearest relevant implementation and tests before editing.
Keep changes focused, use synthetic examples only, and run the narrowest relevant
static parse check after editing. Full tests run later in isolated pull-request CI.
