...
(function () {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (!opId || btn.disabled) return;
    btn.disabled = true;
    try {
      if (window.htmx && typeof window.htmx.ajax === "function") {
        await window.htmx.ajax("POST", `/undo/${encodeURIComponent(opId)}`, { target: "body", swap: "none" });
      } else {
        const response = await fetch(`/undo/${encodeURIComponent(opId)}`, {
      ...
      window.showError("Could not undo the action. Check the connection and try again.");
    } finally {
      btn.disabled = false;
    }
  });
    ...
    document.querySelectorAll(".kanban-column-body.sortable").forEach((col) => {
      new Sortable(col, {
        ...
        onEnd: async (evt) => {
          ...
          try {
            const response = await fetch(`${endpoint}/${uuid}/status`, {
              method: "POST",
            ...
            window.showError("Could not confirm the status change. Check the connection and try again.");
          }
  ...
  function initBulkTasks(root) {
    ...
    const applyButton = bar.querySelector("[data-bulk-apply]");
    applyButton.addEventListener("click", async () => {
      if (applyButton.disabled || !selected.size) return;
      ...
      try {
        ...
        if (!Array.isArray(data.results)) throw new Error("Invalid bulk result");
        ...
        window.showError("Could not confirm the bulk action. Check the connection and try again.");
      } finally {
